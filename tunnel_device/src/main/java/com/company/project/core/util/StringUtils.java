...
public class StringUtils {
	...
	*/
	 ...
	 * @return
	 */
	public static List parseArray(byte[] bytes,Class clzss){
		List<JSONObject> objectList  = (List<JSONObject>) JSON.parse(bytes);
		List list = Lists.newArrayList();
		for(int i=0;i<objectList.size();i++){
			try{
				list.add(JSON.toJavaObject(objectList.get(i),clzss));
			}catch (Exception e){
