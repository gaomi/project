var $MB = (function () {
    var bootstrapTable_default = {
        ...
        pageNumber: 1,
        pageSize: userPageSize ? userPageSize : 10,
    ...
    function _initTreeTable(id, setting) {
        $('#' + id).bootstrapTreeTable({
            ...
            type: !setting.method ? 'POST' : setting.method,  // 请求数据的ajax类型
    ...
    function _closeAndRestModal(modalId) {
        ...
        $modal.find("button.paoding_btn_hide").attr("data-dismiss", "modal").trigger('click');
     ...
     * @private
     */
    function _removeOverlay(classType) {
        $("div." + classType).remove();
    }
     ...
     * @private
     */
    function _initDatepicker(classType) {
    ...
    return {
        ...
        removeOverlay: function (classType) {
            _removeOverlay(classType);

        },
        initDatepicker: function (classType) {
            _initDatepicker(classType);
        },
