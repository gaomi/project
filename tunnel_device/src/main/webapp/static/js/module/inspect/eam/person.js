var $eamPersonSearchForm = $('#eam_person_search_form');
;!function () {
    ...
    var operateEvents = {
        'click .paoding_btn_update': function (e, value, row, index) {
            ...
            $MB.confirm({
                text: "确定重置[" + row.personname + "]的密码？",
                confirmButtonText: "确定"
            }, function () {
                $.post(ctx + '/module/eam/person/resetPassWord', {"id": row.uuid}, function (r) {
                    if (r.code === 200) {
                        $MB.n_success("更新成功");
                        refresh();
                    } else {
    ...
    var settings = {
        url: ctx + "/module/eam/person/list",
    ...
    $MB.initTable('eam_person_table', settings);

}();

function search() {
    $MB.refreshTable('eam_person_table');
}
...
function getAllFromEam() {
    $.post(ctx + '/module/eam/person/getAllFromEam', {}, function (r) {
        if (r.code === 200) {
            $MB.n_success("更新成功");
            refresh();
        } else {
